...
<title>Voxtract — Transcript Inventory</title>
<style>
  :root {{
    --bg: #0f1117;
    --surface: #1a1d27;
...
<p class="meta">Generated by <a href="https://github.com/CosmicDreamer17/youtube-to-polished-transcript">voxtract</a> — manifest.json available in the same directory for programmatic access</p>
  ...
  th.addEventListener('click', () => {{
    ...
    rows.forEach(r => tbody.appendChild(r));
        ...
        table_html = if entries.is_empty() {
            r#"<div class="empty">No transcripts yet. Run <code>voxtract transcribe</code> to get started.</div>"#.to_string()
        } else {
            format!(
                r#"<table>
<thead>
